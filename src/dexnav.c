...
static void CreateTypeIconSprites(void)
{
    ...
    LoadCompressedSpriteSheet(&gSpriteSheet_MoveTypes);//gSpriteSheet_MoveTypes
    LoadCompressedPalette(gMoveTypes_Pal, 0x1D0, 0x60);
    for (i = 0; i < 2; i++)
    {
        if (sDexNavUiDataPtr->typeIconSpriteIds[i] == 0xFF)
            sDexNavUiDataPtr->typeIconSpriteIds[i] = CreateSprite(&gSpriteTemplate_MoveTypes, 10, 10, 2);  //gSpriteTemplate_MoveTypes  
